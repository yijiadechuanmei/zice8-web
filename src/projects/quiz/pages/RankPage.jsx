    ...
    <main className="quiz-page flex min-h-screen w-full justify-center bg-[#143978] pb-7">
      <section className="w-full max-w-[750px]">
        <DesignStage height={1624}>
          ...
          <div className="absolute left-[64px] top-[342px] h-[900px] w-[618px] text-[#173f2a]">
            ...
            {ranks.length ? (
              <div className="flex h-full flex-col">
                <div className="grid h-[54px] grid-cols-[64px_150px_150px_92px_120px] items-center gap-[8px] px-[10px] text-[18px] font-extrabold text-[#fff6d3]">
                  <span>排名</span>
                  <span>姓名</span>
                  <span>部门</span>
                  <span>积分</span>
                ...
                <div className="mt-[14px] flex flex-col gap-[10px] overflow-auto">
                  {ranks.map((item) => (
                    <div
                      className={`grid h-[58px] grid-cols-[64px_150px_150px_92px_120px] items-center gap-[8px] rounded-2xl px-[10px] text-[18px] ${
                        item.rank <= 3 ? 'bg-[rgba(249,242,181,0.96)]' : 'bg-[rgba(255,250,237,0.92)]'
                      }`}
                      key={`${item.rank}-${item.userId}`}
                    >
                      <strong className="text-center text-[28px] font-black text-[#8a5d00]">{item.rank}</strong>
