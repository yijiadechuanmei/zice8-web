...
const DEFAULT_PAGE = 'result'
const VALID_PAGES = new Set(['question', 'result', 'wheel'])
const DRAW_STATUS = {
  WON: 'won',
  LOST: 'lost',
...
function inferPage(model) {
  if (model?.state === 'answering') return 'question'
  if (model?.alreadyDrawn || model?.draw) return 'wheel'
  return DEFAULT_PAGE
}
...
function PhaseQuizLotteryMain({ routeParams }) {
  ...
  const initialPage = sanitizePage(routeParams?.page) || sanitizePage(getCurrentPathPage()) || ''
  ...
  const explicitPageRef = useRef(Boolean(initialPage))
  ...
  function updateHistory(nextPage, replace = false) {
    const target = `/phase-quiz-lottery/${encodeURIComponent(activityKey)}/${nextPage}`
    if (replace) {
  ...
  function applyResolvedPage(nextPage, replace = false) {
    setPage(nextPage)
    if (activityKey) updateHistory(nextPage, replace)
  }

  function applyResultModel(nextModel, options = {}) {
    setModel(nextModel)
    setAttemptId(nextModel?.attempt?.attemptId || attemptId)
    setDraw(nextModel?.draw || null)
    if (options.navigate !== false) {
      applyResolvedPage(options.page || inferPage(nextModel), options.replace)
    }
  }
  ...
  useEffect(() => {
    function handlePopState() {
      const nextPage = sanitizePage(getCurrentPathPage()) || inferPage(model)
      setPage(nextPage)
    }
  ...
  useEffect(() => {
    ...
    async function loadBootstrap() {
      ...
      try {
        ...
        const targetPage = explicitPageRef.current && initialPage
          ? initialPage
          : inferPage(data)
        applyResolvedPage(targetPage || DEFAULT_PAGE, true)
      } catch (error) {
  ...
  useEffect(() => {
    if (!activityKey || !page) return
    trackPageView(activityKey, `/phase-quiz-lottery/${page}`, {
      activityType: 'phase_quiz_lottery',
      pageKey: page,
      phaseNo: model?.currentPhase?.phaseNo || null,
    })
  }, [activityKey, model?.currentPhase?.phaseNo, page])
    ...
    try {
      ...
      if (data.state === 'answering') {
        startTransition(() => {
          ...
          applyResolvedPage('question')
        })
        return
      }

      applyResultModel(data, { page: 'result' })
    } catch (error) {
  ...
  async function handleGoWheel() {
    if (!attemptId && model?.attempt?.attemptId) {
      try {
        ...
        if (resultData) {
          applyResultModel(resultData, { page: 'wheel' })
          return
        }
      } catch (error) {
  ...
  async function handleDraw() {
    ...
    try {
      ...
      } else if (data.status === DRAW_STATUS.STOCK_EMPTY) {
        showToast('本期已抽完')
        setDrawing(false)
      }
    } catch (error) {
