...
function PhaseQuizLotteryMain({ routeParams }) {
  ...
  async function handleDebugResetAll() {
    ...
    const confirmed = window.confirm('确认重置第3期测试数据？会清空第3期所有用户的答题、抽奖、兑奖记录，并恢复第3期奖品库存。')
    if (!confirmed) return

    setLoading(true)
    setLoadingText('正在重置第3期测试数据...')
    try {
      ...
      console.log('[phase-quiz-lottery debug reset phase 3]', result)
      showToast('第3期测试数据已重置')
      setQuestions([])
      setCurrentIndex(0)
      setAnswers([])
      setAttemptId('')
