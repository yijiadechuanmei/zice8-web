...
export default function DebugPanel({ activityKey, status, bootstrap, onClose }) {
  ...
  const rows = [
    ...
    ['splashShown', status.splashShown],
    ['splashAutoEnterMs', status.splashAutoEnterMs],
    ['splashImageUrl', status.splashImageUrl],
    ['snapshotUser', status.snapshotUser],
    ['rawNickname', status.rawNickname],
    ...
    ['requireUserinfo', status.requireUserinfo],
    ['autoAuthStarted', status.autoAuthStarted],
    ['snapshot_user 参数', status.snapshotUserParam],
    ['授权前已加载视频', status.videoListLoadedBeforeAuth],
