...
export default function VideoCard({ video, onClick }) {
  ...
  return (
    <button onClick={() => video.unlocked && onClick(video)} className="w-full cursor-pointer overflow-hidden rounded-2xl border border-slate-100 bg-white text-left shadow-sm transition-shadow duration-200 hover:shadow-md disabled:cursor-not-allowed disabled:opacity-70" disabled={!video.unlocked}>
